	...
	namespace write_vts {
		...
		void write_vts_pos_iii() {
			...
			if (main_iterator::Current_ITE_step % output_frequence == 0) {
				std::ofstream fout;
				// - 
				default_functions::open_vts_scalar_file(fout, "step" + std::to_string(main_iterator::Current_ITE_step));
				for (auto writer = write_vts_scalar_list.begin(); writer < write_vts_scalar_list.end(); writer++)
					(*writer)(fout);
				default_functions::close_vts_file(fout);
				// - 
				default_functions::open_vts_vec3_file(fout, "step" + std::to_string(main_iterator::Current_ITE_step));
				for (auto writer = write_vts_vector_list.begin(); writer < write_vts_vector_list.end(); writer++)
					(*writer)(fout);
				default_functions::close_vts_file(fout);
			}
