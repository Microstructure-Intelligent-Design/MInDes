...
namespace pf {
	...
	namespace elastic_solver {
		...
		static int restart_iterator_in_loop_steps = 1;
		// plastic solver
		...
		static string exec_loop_im_steinbach(FieldStorage_forPhaseNode& phaseMesh) {
			...
			mechanical_field_solver_im.cal_parameters_before_calculation(is_plastic_on);
			if (restart_iterator_in_loop && Solvers::get_instance()->current_istep % restart_iterator_in_loop_steps == 0)
				mechanical_field_solver_im.initStrainIncrements();
			int plastic_iteration = 0, map_steps = 0;
			double dplastic_strain = 0.0, dave_plastic_strain = 0.0;
			string elastic_solver_output = "";
		...
		static void init() {
			...
			if (MechanicalFieldType(MFType) == MechanicalFieldType::MFType_Explicit) {
				...
				if (InputFileReader::get_instance()->read_bool_value("Postprocess.SolidMechanics.restart_iterator_in_loop", restart_iterator_in_loop, infile_debug))
					InputFileReader::get_instance()->read_int_value("Postprocess.SolidMechanics.RestartIteratorInLoop.steps", restart_iterator_in_loop_steps, infile_debug);
