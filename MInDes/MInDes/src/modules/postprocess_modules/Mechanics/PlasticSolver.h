...
namespace pf {
	namespace plastic_solver {
		...
		static double get_hardening_modulus_acc(PhaseNode& node) {
			double H = 0.0;
			for (auto phase = node.ActivePhase.begin(); phase < node.ActivePhase.end(); phase++)
				H += (*phase)->phi * phi_hardening_modulus((*phase)->property);
			return H;
		}
		static double get_hardening_modulus_norm(PhaseNode& node) {
			double H = 0.0;
...
#pragma omp parallel for
		...
		static void solve_plastic_flow_norm(FieldStorage_forPhaseNode& phaseMesh, int& max_iteration_steps, double& max_dplastic_strain, double& max_dave_plastic_strain) {
			// sqrt(2.0 / 3.0) = 0.8164965809
...
#pragma omp critical
		...
		static void solve_plastic_flow_acc(FieldStorage_forPhaseNode& phaseMesh, int& max_iteration_steps, double& max_dplastic_strain, double& max_dave_plastic_strain) {
			// sqrt(2.0 / 3.0) = 0.8164965809
			max_iteration_steps = 0;
			max_dplastic_strain = 0.0;
			max_dave_plastic_strain = 0.0;
...
#pragma omp critical
		...
		static void init() {
			...
			if (Solvers::get_instance()->parameters.is_pair_wise_accelerate)
				solve_plastic_flow = solve_plastic_flow_acc;
			else
				solve_plastic_flow = solve_plastic_flow_norm;
			if (is_plastic_on) {
