...
namespace pf {
	...
	namespace infile_math_default_funcs {
		...
		REAL rand_int(vector<int>& para_index, vector<REAL>& para_vals
			...
			REAL result = (para_vals[1] - para_vals[0]) * rand + para_vals[0];
			if (result > 0.0)
				return REAL(int(result + 0.5));
			else
				return REAL(int(result - 0.5));
		}
		REAL rand_real(vector<int>& para_index, vector<REAL>& para_vals
			...
			std::uniform_real_distribution<> real_dist(0.0, 1.0); // [0.0, 1.0] 范围内的浮点数
			REAL rand = REAL(real_dist(gen));  // random
			return (para_vals[1] - para_vals[0]) * rand + para_vals[0];
		}
		vector<REAL> formula(vector<REAL> terms, vector<CalculationOperator> operators) {
			...
			if (is_ParaSeparator_exist) {
				if (split_terms.size() == 0 || split_operators.size() == 0) {
					cout << "> formula error, no formula on the left of \',\'!" << endl;
					SYS_PROGRAM_STOP;
				}
				vector<REAL> cal_values = formula(split_terms, split_operators);
				return_vals.push_back(cal_values[0]);
	...
	bool InfileMath::search_var(string var_key, int& var_index) {
	...
	bool InfileMath::search_func(string func_key, int& func_index) {
	...
	int InfileMath::add_infile_var(string key, REAL var) {
	...
	bool InfileMath::check_default_func(string key) {
		if (key.compare("") == 0)
			return false;
	...
	bool InfileMath::check_variables_funcs_keys() {
	...
	int InfileMath::add_default_func(string key) {
		if (key.compare("") == 0)
			return false;
		...
		case pf::FType_atan:
			...
			break;
		case pf::FType_rand_int:
			newFunc.func = infile_math_default_funcs::rand_int;
			break;
		case pf::FType_rand_real:
			newFunc.func = infile_math_default_funcs::rand_real;
			break;
		default:
	...
	string InfileMath::split_equation_str_by_separators_and_operators(string equation, vector<char> separators, vector<string>& formulas
	...
	string InfileMath::split_field_variable_by_separators_and_operators(string equation, vector<char> separators, vector<string>& formulas
		, vector<string>& formula_keys, vector<CalculationOperator>& cal_operators) {
		bool is_split = false, is_pair_seperators = true;
		// check equation degree
		if (separators.size() != 0) {
