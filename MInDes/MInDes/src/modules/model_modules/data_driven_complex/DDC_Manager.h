	...
	namespace data_driven_complex_model {
		...
		*/
		inline void init_model_modules() {
				...
				if (infile_reader::read_real_value("Model.DDC.Phi.IntMobilityQ.const", const_input, true)) {
					...
					if (InputFileReader::get_instance()->read_string_value(const_block_key, const_block_input, true)) {
						...
						for (int block_index = 0; block_index < const_block_value.size(); block_index++)
							for (auto phi_a_index = const_block_value[block_index][0].begin(); phi_a_index < const_block_value[block_index][0].end(); phi_a_index++)
								for (auto phi_b_index = const_block_value[block_index][1].begin(); phi_b_index < const_block_value[block_index][1].end(); phi_b_index++) {
									data_driven_complex_model::parameters::Qij(phi_a_index->int_value, phi_b_index->int_value) = const_input;
									data_driven_complex_model::parameters::Qij(phi_b_index->int_value, phi_a_index->int_value) = const_input;
								}
					}
					else {
						for (size_t phi_a_index = 0; phi_a_index < main_field::phi_number; phi_a_index++)
			...
			load_a_new_module(nullptr, data_driven_complex_model::exec_pre_ii, data_driven_complex_model::exec_pre_iii,  // exec_pre_i   exec_pre_ii    exec_pre_iii
				data_driven_complex_model::exec_i, nullptr, nullptr,  // exec_i   exec_ii   exec_iii
				data_driven_complex_model::exec_pos_i, nullptr, nullptr,   // exec_pos_i   exec_pos_ii   exec_pos_iii
