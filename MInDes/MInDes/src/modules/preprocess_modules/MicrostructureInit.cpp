		...
		namespace functions {
			void init_mesh_with_datafile(pf::Data_MeshInfo& report, std::string dat_path) {
				if (!write_mesh_data::read_dataFile(dat_path, report)) {
					std::string _report = "> Error : datafile_path can't be opened ! \n";
					WriteLog(_report);
					std::exit(0);
				}
				else {
					// check datafile with this input file
					std::vector<std::string> bool_type; bool_type.push_back("MISMATCH"); bool_type.push_back("MATCH");
					...
					if (all_valid == false) {
						std::string error_report = "> Wainning : mesh data structure from datafile and inputfile mismatch ! \n";
		...
		void init_microstructure() {
			if (main_field::is_phi_field_on || main_field::is_con_field_on || main_field::is_temp_field_on) {
				...
				if (write_mesh_data::is_datafile_init) {
					...
					std::replace(write_mesh_data::datafile_path.begin(), write_mesh_data::datafile_path.end(), '\\', '/');
#ifdef _WIN32
				...
				// - others
				load_a_new_module(init_microstructure_pre_i, nullptr, nullptr,
				...
				if (infile_reader::read_int_value("Preprocess.Microstructure.Output.frequence", write_mesh_data::output_frequence, true)) {
					if (write_mesh_data::output_frequence == 0) {
						load_a_new_module(nullptr, nullptr, write_data_pre_iii,
							nullptr, nullptr, nullptr,
							nullptr, nullptr, nullptr, nullptr);
					}
						...
						load_a_new_module(nullptr, nullptr, write_data_pre_iii,
							nullptr, nullptr, nullptr,
							nullptr, nullptr, write_data_pos_iii, nullptr);
					}
				}
			}
		}
